...
export function HomeView({ setView, setActiveIndex, setFilteredStations, allStations }: HomeViewProps) {
  const menuItems = [
          ...
          {menuItems.map((item, index) => (
            <li key={item.label}>
              <button
                ...
                className="w-full text-left p-3 rounded-lg bg-background hover:bg-primary/10 transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
              >
                <span className="font-bold text-primary">{index + 1}.</span> {item.label}
              </button>
            </li>
          ))}
        </ul>
